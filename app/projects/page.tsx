...
export default async function ProjectsPage() {
  ...
  const languages = Array.from(new Set(repos.map((r) => r.language ?? "Other"))).sort();
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-12 sm:px-6 sm:py-16">
      ...
      {repos.length === 0 ? (
        <div className="rounded-xl border border-dashed border-slate-300 dark:border-slate-600 p-16 text-center text-slate-400 dark:text-slate-500">
          Could not load repositories. Add a{" "}
          <code className="font-mono text-sm">GITHUB_TOKEN</code> to your environment variables.
        </div>
      ) : (
        <>
          <div className="mb-8 flex flex-wrap gap-2">
            {languages.map((lang) => {
              const count = repos.filter((r) => (r.language ?? "Other") === lang).length;
              return (
                <span key={lang}
                  ...
                  {lang} <span className="text-slate-400 dark:text-slate-500">({count})</span>
                </span>
              );
            })}
          </div>
